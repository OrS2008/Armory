...
CREATE TABLE assignment_type_crews (
  id TEXT PRIMARY KEY,
...
CREATE UNIQUE INDEX idx_crew_name ON assignment_type_crews(assignment_type_id, name);
CREATE INDEX idx_crew_post ON assignment_type_crews(assignment_type_id, position);
...
CREATE TABLE assignment_type_crew_members (
...
CREATE INDEX idx_crew_member ON assignment_type_crew_members(personnel_id);
...
SELECT 'atp_hafak', q.id, 1
  FROM qualifications q
 WHERE q.org_id = 'org_default' AND q.code IN ('CMD','DRIVER','MEDIC','MARKSMAN');
