...
export function InventoryLoansPage() {
  const client = useQueryClient();
  const [dialog, setDialog] = useState(false);
  const assets = useQuery({ queryKey: ['assets', 'inventory'], queryFn: () => api.assets('inventory') });
  const loans = useQuery({ queryKey: ['equipment-loans'], queryFn: api.equipmentLoans });
  const refresh = async () => Promise.all([client.invalidateQueries({ queryKey: ['equipment-loans'] }), client.invalidateQueries({ queryKey: ['assets', 'inventory'] })]);
  ...
  return <section className="page compact-page">
    <header className="page-title"><div><span className="eyebrow">מלאי תפעולי</span><h1>השאלות ציוד</h1><p>מסירה, יעד, כמות והחזרה שמעדכנת את המלאי.</p></div><Button variant="primary" onClick={() => setDialog(true)} disabled={!assets.data?.items.length}><Plus />השאלה חדשה</Button></header>
